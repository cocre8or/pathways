 ...
 */
var RegularLabsTabs=null;(function($){"use strict";$(document).ready(function(){var options=typeof(window['rl_tabs_options'])!=="undefined"?rl_tabs_options:Joomla.getOptions('rl_tabs');if(typeof(options.init_timeout)=="undefined"){return;}
setTimeout(function(){RegularLabsTabs.init(options);},options.init_timeout);});RegularLabsTabs={options:{},timers:[],stop_timers:[],scroll_to:null,scrolling:false,init:function(options){var self=this;this.options=options;try{this.hash_id=decodeURIComponent(window.location.hash.replace('#',''));if(this.hash_id.indexOf('/')>-1||this.hash_id.indexOf('/')>-1){this.hash_id='';}}catch(err){this.hash_id='';}
...
var id=$link.attr('href').substr($link.attr('href').indexOf('#')+1);if(id==''){return;}var scroll=this.options.linkscroll;var is_tab=true;var $anchor=$('a[data-toggle="tab"][data-id="'+id+'"]');if(!$anchor.length){$anchor=$('#'+id+',a[name="'+id+'"]');if(!$anchor.length){return;}
